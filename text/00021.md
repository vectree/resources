...
Java - является [популярым и востребованным](https://www.tiobe.com/tiobe-index/) языком программирования, а заработная плата Java-разработчика одна из самых высоких на рынке. Связано это с тем, что язык активно используется для создания **надежных** корпоративных решений как крупными, так и небольшими компаниями.
...
Загрузите последнию версию Java 8 с [официального сайта](http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html). Здесь вы можете столкнуться с вопросом, какой из файлов скачать, JDK или JRE? 
...
Java - довольно безопасный язык, так как он не позволяет вашей программе работать непосредственно с вашим компьютером. Вместо этого ваша программа запускается на виртуальной машине - JVM. По сути JVM - это защитная прослойка между программой и вашим компьютером, через которую проходят только те команды, которые разрешены, предоставлены ограниченным [API](https://ru.wikipedia.org/wiki/API).
...
Надеюсь у вас все получилось? Если нет, прочитайте данную статью о создании проекта в [Intellij Idea](https://metanit.com/java/tutorial/1.5.php) или напишите нам на [чат](https://discord.gg/eNVAxeE), на [форум](https://forum.vectree.ru).
...
Запуск через IDE - это круто! Все просто и понятно - жманул на Run и код выполнился. Но что, если у нас нет IDE? Как тогда запустить программу? Ответ прост: через терминал.
